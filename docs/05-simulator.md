...
inside its node, elsewhere in its supernode and in another one, and compose from per-tier
...
all-to-all spanning both supernodes; the measurement is 33.9. Reading the implied tier
bandwidth back out of each configuration says why:

| tier | at 8 peers | at many peers |
|---|---:|---:|
| cross-node | 103.0 GB/s | 118.9 GB/s at 120 |
| cross-supernode | 13.3 GB/s | 20.0 GB/s at 64 |

That reading — **a tier delivers more per card when its bytes are spread over more
peers** — was then tested directly and **is wrong**. Measured with split sizes arranged
so every card sends only across the boundary, with nothing to deconvolve, per-card
cross-supernode bandwidth is 7.82 GB/s at one peer and 8.03 at sixty-four: flat to 2.6%
over a sixty-fourfold change in spread. The dependence above is an artefact of the
deconvolution, not a property of the fabric.

Two consequences, pointing opposite ways. The charge this raised against two-hop is
**withdrawn** — Hop A runs at the fewest peers of anything in the scheme, and pays
nothing for it. And the 31% out-of-sample miss still stands with no explanation, which
now indicts the deconvolution itself: the one step in this analysis that assumes
anything.

One variable does survive, from a different comparison: holding peers at eight and
varying only how many nodes cross concurrently, cross-supernode falls from 25.9 GB/s
alone to 13.3 in company. Concurrent load is real; peer spread is not.
...
one-hop, and hardest where the hierarchy is coarsest — a two-supernode job gives Hop A a
single cross-supernode peer. That was named as the first thing to measure before the
boundary result in [docs/03](03-applicability.md) could be treated as settled, and the
measurement above is that test: **the effect is not there, so the charge is withdrawn.**
