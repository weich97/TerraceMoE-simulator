# -*- coding: utf-8 -*-
"""生成 docs/assets/ 下的全部结果图(SVG,文字走客户端字体,GitHub 直接渲染)。

**本脚本内嵌的数字就是发布的数字** —— 图、docs/06 的表、本脚本三处必须一致;
数字口径均来自内部实测记录,汇总口径在 docs/06-troute-results.md 各节标注。
跑法:python tools/gen_figures.py   (需要 matplotlib)
"""
...
plt.rcParams.update({
    "svg.fonttype": "none",           # 文字保留为 <text>,浏览器用客户端字体渲染中文
...
# ---------------------------------------------------------------- F1 头条
# 13.14B/A1.33B,4 模式 x 4 种子,留出集 val loss,配对差 vs 无约束,90% CI
...
x = range(3)
for i, (dat, off, lab) in enumerate([(D20, -0.17, "读点 @20k 步"),
                                     (D30, +0.17, "读点 @30k 步")]):
    xs = [j + off for j in x]
...
ax.text(2.46, 0.0205, "预注册等价容差 0.02", color="#B33", ha="right", fontsize=9)
ax.axhspan(0.0056, 0.0058, color="#888", alpha=0.35)
ax.text(2.46, 0.0060, "训练日志时代的种子噪声 0.0056–0.0058", color="#555",
        ha="right", fontsize=8.5)
ax.set_xticks(list(x), MODES3)
ax.set_ylabel("Δ val loss vs 无约束 top-k(nats,越低越好)")
ax.set_title("质量代价:T-Route +0.0034 nats,为容差的 1/5(判据 0.1 在图外 5 倍处)")
ax.legend(frameon=False, loc="upper left")
_style(ax)
fig.tight_layout()
fig.savefig(os.path.join(OUT, "f1-loss-axis.svg"))
plt.close(fig)

# ---------------------------------------------------------------- F2 逐种子
# 24 个配对差(3 约束档 x 4 种子 x 2 读点)全部为正 —— 效应真实且小
SEED = {
...
for j, k in enumerate(CK):
    for rp, mk in (("20k", "o"), ("30k", "s")):
        ax.scatter([j + (-0.08 if rp == "20k" else 0.08)] * 4, SEED[rp][k],
                   ...
                   label=("读点 @%s" % rp) if j == 0 else None)
ax.axhline(0, color="#333", lw=1)
ax.set_xticks(range(3), MODES3)
ax.set_ylabel("逐种子配对差(nats)")
ax.set_title("24/24 个配对差同号为正:差异真实、且每一个都远小于容差")
ax.legend(frameon=False, loc="upper left")
_style(ax)
fig.tight_layout()
fig.savefig(os.path.join(OUT, "f2-per-seed.svg"))
plt.close(fig)
...
ax.axhline(-1.0, color=C["full"], ls="--", lw=1)
ax.text(1.45, 1.06, "预注册等价边界 ±1.0 pp", color=C["full"], ha="right", fontsize=9)
...
ax.set_xlim(-0.5, 1.5)
ax.set_ylim(-1.4, 1.4)
ax.set_ylabel("T-Route Δ vs 无约束(pp,90% CI)")
ax.set_title("下游:两轴均等价(TOST)")
_style(ax)

# LAMBADA 三读点摆动:单读点会骗人
...
ax2.set_title("单读点会骗人:三读点摆动 1–2 pp 且变号 = 噪声")
ax2.legend(frameon=False, fontsize=8.5)
_style(ax2)
fig.tight_layout()
fig.savefig(os.path.join(OUT, "f3-downstream.svg"))
plt.close(fig)
...
for a, dat, ttl, better in ((ax, ENT, "专家级负载熵 Δ(高=更均衡)", "≥0 即无隐性容量损失"),
                            (ax2, CV, "专家级负载 CV Δ(低=更均衡)", "≤0 即更均衡")):
    ys = [dat[k][0] for k in CK]
    ...
    a.set_title(ttl + "\n" + better, fontsize=10)
    _style(a)
fig.suptitle("负载轴:三个约束档相对无约束都不更差(配对,n=4)", y=1.02)
fig.tight_layout()
...
ax.text(1.44, 1.024, "同型作业发间散布带 ±3%", color="#555", ha="right", fontsize=8.5)
...
ax.text(1.44, sum(runs) / 2 - 0.004, "均值 0.9976", color=C["full"], ha="right", fontsize=9)
ax.set_xticks([0, 1], ["第 1 发", "第 2 发"])
ax.set_xlim(-0.45, 1.5)
ax.set_ylim(0.955, 1.045)
ax.set_ylabel("G = t(无约束) / t(T-Route)")
ax.set_title("步时:两臂同用基线 a2a、只差路由 —— T-Route 不收步时税")
_style(ax)
fig.tight_layout()
fig.savefig(os.path.join(OUT, "f5-step-neutral.svg"))
plt.close(fig)
...
ax.set_ylabel("留出集 val loss")
ax.set_title("前哨(小规模合成,2×2 分解):四档 loss 差 ≤0.0011 < 种子极差", fontsize=10)
_style(ax)
...
ax2.set_ylabel("负载熵(高=均衡)")
ax2.set_title("同一试验台:full 的负载熵最高", fontsize=10)
_style(ax2)
fig.tight_layout()
fig.savefig(os.path.join(OUT, "f6-forerunner.svg"))
plt.close(fig)
