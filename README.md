...
| `terrace/ta2a*.py` | T-A2A two-hop chain: planning, dispatch, packing, differentiable seam | **Validated bit-exact** (guarded by the repo's 385 CPU tests; end-to-end measured only on a flat supernode, see the criterion) |
...
| `tests/` | 385 tests, pure CPU (no NPU needed); `test_docs_numbers.py` recomputes every derived number these docs state, and `test_cli_output_portable.py` runs each documented command against a console that cannot encode anything but ASCII | All green |
