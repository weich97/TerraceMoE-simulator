# TerraceMoE:面向层级化集群的 MoE 路由与通信

**T-Route**(层级对齐的路由约束)+ **T-A2A**(两跳分层 all-to-all)的方法、参考实现与适用判据。
...
13.14B 总参 / 1.33B 激活规模,四档消融(4 模式 × 4 种子,每臂 62.9B token,留出集 val loss):
...
| `terrace/ta2a*.py` | T-A2A 两跳链:计划、派发、打包、可微接缝 | **已验证位级正确**(由全仓 267 条 CPU 测试把守;端到端只在扁平超节点测过,见判据) |
...
| `tools/breakeven.py` | 适用判据 | — |
| `tests/` | 267 条测试,纯 CPU 可跑(不需要 NPU) | 全绿 |
| `docs/` | 设计文档 ×4 | — |
