...
> **rack boundary measures 2.55** (`python -m sim.hierarchy`), the first measured ratio
> here above 1.03; it clears the byte criterion and lands between the fused and
> operator-chain thresholds, so there the arrival chain decides. That is a measured
> ratio, not a calibrated platform: expert parallelism across two racks is 256 cards and
> alpha past world 128 is unsupported.
...
| `terrace/ta2a*.py` | T-A2A two-hop chain: planning, dispatch, packing, differentiable seam | **Validated bit-exact** (guarded by the repo's 395 CPU tests; end-to-end measured only on a flat supernode, see the criterion) |
