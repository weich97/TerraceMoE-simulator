...
| `terrace/ta2a*.py` | T-A2A two-hop chain: planning, dispatch, packing, differentiable seam | **Validated bit-exact** (guarded by the repo's 281 CPU tests; end-to-end measured only on a flat supernode — see the criterion) |
...
| `sim/` | Measurement-calibrated communication simulator: cluster spec → one-hop/two-hop times, breakeven maps, Monte Carlo uncertainty bands (see [docs/05-simulator.md](docs/05-simulator.md)) | **Tier-1 (communication micro) validation gate passed** (median error 3.6%); Tier-2 (end-to-end step level) honestly marked as failing, step-level extrapolation banned — the systematic negative result across six families of overlap models, and the measurement protocol that would break the impasse, are in [docs/07-tier2-overlap.md](docs/07-tier2-overlap.md) |
...
| `tests/` | 281 tests, pure CPU (no NPU needed) | All green |
